...
[ -z "${OPT_SECTIONS}" ] && OPT_SECTIONS="apt_update clean_packages clean_libraries preconfig_udev_ifnames preconfig_initramfs preconfig_grub preconfig_fstab preconfig_hosts apt_dist_upgrade clean_apt clean_history"
...
function _clean_packages {
  ...
  local debs_di='aptitude aptitude-common avahi-autoipd blends-tasks bluetooth bluez dictionaries-common discover discover-data dmidecode eject emacsen-common iamerican ibritish ienglish-common installation-report ispell laptop-detect nano os-prober task-english tasksel tasksel-data wbritish wamerican'
