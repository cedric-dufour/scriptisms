...
# REF: https://github.com/cedric-dufour/scriptisms/blob/master/system/packer/bookworm/provision.sh
SCRIPT="${0##*/}"
VERSION='2023.04.05a'
...
[ -z "${OPT_SECTIONS}" ] && OPT_SECTIONS="apt_update usrmerge clean_packages clean_orphans clean_ghosts preconfig_udev_ifnames preconfig_initramfs preconfig_grub preconfig_fstab preconfig_hosts apt_dist_upgrade clean_apt clean_history"
...
function _clean_packages {
  ...
  local debs_di='apt-utils bsdextrautils bsdmainutils calendar cpp cpp-11 cpp-12 debconf-i18n dictionaries-common discover discover-data dmidecode eject emacsen-common gcc-11-base qemu-guest-agent iamerican ibritish ienglish-common installation-report intel-microcode ispell iucode-tool laptop-detect nano ncal os-prober pci.ids pciutils perl shared-mime-info task-english tasksel tasksel-data usrmerge util-linux-locales wamerican xauth xdg-user-dirs'
